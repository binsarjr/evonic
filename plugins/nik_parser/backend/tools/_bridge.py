...
def module(name: str):
    return importlib.import_module(f"plugins.nik_parser.{name}")


def read_agent_image(agent: dict, args: dict):
    path = args.get("path")
    path = path.strip() if isinstance(path, str) else ""
    if not path: raise ValueError("IMAGE_PATH_REQUIRED")
    ...
    if not mime: raise ValueError("UNSUPPORTED_IMAGE_TYPE")
