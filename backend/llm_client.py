...
class LLMClient:
    ...
    def chat_completion(
        ...
        provider = get_provider({"api_format": self.api_format, "base_url": self.base_url})
        url = provider.base_url + provider.completion_path
        if max_tokens is None:
            max_tokens = self.max_tokens
        ...
        for attempt in range(1 + max_retries):
            try:
                ...
                result = provider.normalize_response(result)
