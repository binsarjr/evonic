...
def test_anthropic_uses_advertised_levels_only():
    provider = get_provider({'api_format': 'anthropic', 'base_url': 'https://api.anthropic.com/v1'})
    metadata = {'capabilities': {'effort': {'supported': True, 'low': {'supported': True},
                'high': {'supported': True}, 'max': {'supported': False}}}}
    assert provider.get_reasoning_capabilities('claude', metadata) == {
        'efforts': ['low', 'high'], 'default_effort': 'high',
    }
    assert provider.get_reasoning_capabilities('claude')['efforts'] == []
    payload = {'output_config': {'format': {'type': 'json_schema'}}}
    provider.apply_reasoning_effort(payload, 'high')
...
def test_gateway_and_other_formats_do_not_inherit_deepseek_support(url, api_format):
    provider = get_provider({'base_url': url, 'api_format': api_format})
    assert provider.get_reasoning_capabilities('deepseek-v4-pro')['efforts'] == []
